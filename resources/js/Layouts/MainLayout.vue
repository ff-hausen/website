        ...
        <Disclosure as="nav" class="bg-red-600" v-slot="{ open }">
            <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                <div class="flex h-16 items-center justify-between">
                    <div class="flex items-center">
                        <div class="flex-shrink-0">
                            <Link href="/" class="flex items-center">
                                <img class="h-10" src="/images/ffhausen-logo.png" aria-hidden="true"/>
                                <div
                                    class="hidden md:block ml-2 flex flex-col justify-center text-white font-black leading-tight text-justify">
                        ...
                        <div class="hidden md:block">
                            <div class="ml-10 flex items-baseline space-x-4">
                                <Link v-for="item in navigation" :key="item.name" :href="item.href"
                                      :class="[item.current ? 'bg-red-700 text-white' : 'text-white hover:bg-red-500 hover:bg-opacity-75', 'rounded-md px-3 py-2 text-sm font-medium']"
                                ...
                                <transition enter-active-class="transition ease-out duration-100"
                                    ...
                                    <MenuItems
                                        ...
                                        <MenuItem v-for="item in userNavigation" :key="item.name" v-slot="{ active }">
                                            <Link :href="item.href" :method="item.method" as="button" type="button"
                    ...
                    <div class="hidden md:block" v-else>
                        <Link :href="route('login')" class="block px-4 py-2 text-sm text-gray-200">Mitgliederbereich
                        </Link>
                    </div>
            ...
            <DisclosurePanel class="md:hidden">
                <div class="space-y-1 px-2 pb-3 pt-2 sm:px-3">
                    <DisclosureButton v-for="item in navigation" :key="item.name" as="Link" :href="item.href"
                                      :class="[item.current ? 'bg-red-700 text-white' : 'text-white hover:bg-red-500 hover:bg-opacity-75', 'block rounded-md px-3 py-2 text-base font-medium']"
                                      :aria-current="item.current ? 'page' : undefined">{{ item.name }}
                    </DisclosureButton>
                </div>
                <div class="border-t border-red-700 pb-3 pt-4" v-if="user">
                    ...
                    <div class="mt-3 space-y-1 px-2">
                        <Link v-for="item in userNavigation" :key="item.name" :href="item.href" :method="item.method"
                ...
                <div class="border-t border-red-700 pb-3 pt-4" v-else>
                    <div class="mt-3 space-y-1 px-2">
                        <Link :href="route('login')"
                              class="text-white hover:bg-red-500 hover:bg-opacity-75block rounded-md px-3 py-2 text-base font-medium">
                            Mitgliederbereich
                        </Link>
                    </div>
                </div>
